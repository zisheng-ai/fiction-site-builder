...
- **The cover is an ad creative first, a book cover second.** Design and generate to stop the scroll in 0.3 seconds. For traffic fiction, the default visual is a melodramatic short-drama thumbnail: a concrete conflict, exaggerated readable expressions, and a complicated relationship tableau. Use no more than 4 discernible people; when the premise supports an ensemble conflict, prefer 3–4 clearly differentiated figures instead of a crowded cast. Do not default to prestige aesthetics, cinematic movie posters, beautiful couple portraits, romantic embraces, or polished billionaire-family group photos. See `cover-allure-elements.md` and `facebook-ads.md`.
- **Danger does not mean dark exposure.** Kidnapping, mafia, bullying, crime, thriller, horror, and revenge covers must not automatically default to night scenes, black wardrobes, deep shadow, or near-black backgrounds. Preserve a supplied reference image's luminance and whitespace as first-class composition features. Across a cover batch, vary high-key, mid-key, and low-key treatments; never deliver an all-dark row. Apply the measurable brightness and overlay gates in `story-cover.md` before accepting any cover.
