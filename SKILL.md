...
- **If configured**: add the Facebook Pixel base code to `src/app/layout.tsx` `<head>` using `next/script` with `strategy="afterInteractive"`. Use the exact Pixel ID from `fictions/AGENTS.md`. Also wire the `ChapterPixel` component (see `references/adsense-arbitrage.md §4.3`) into `src/app/book/[slug]/chapter/[n]/page.tsx` — this fires `ViewContent` on mount, `{subdomain}_ScrollDepth25` / `{subdomain}_ChapterRead50` / `{subdomain}_ScrollDepth75` at scroll milestones, `{subdomain}_ChapterCompleted` via IntersectionObserver on `#chapter-content-end` sentinel, and `{subdomain}_TimeOnPage20s` / `{subdomain}_TimeOnPage30` after 20 / 30 seconds. Both dwell events must use the same chapter-only scope, payload, prefix, and lifecycle. These are the L1 optimization signals for Lookalike Audience building.
  - Treat the existing browser Pixel as a production contract. A CAPI, consent, cookie-banner, analytics-helper, or route-tracking change must not remove the layout bootstrap, move it behind a new client/localStorage gate, or make browser `fbq` delivery depend on a CAPI endpoint. Preserve the current site's consent semantics unless the user explicitly scopes a consent migration; then test pre-consent, accept, reject, and returning-accepted states separately.
  - Before changing an existing Pixel path, record its Pixel ID, `layout.tsx` bootstrap and matching `noscript` ID, navigation model, event emitters, consent behavior, and exact custom-event names. Afterward run the regression matrix in `references/adsense-arbitrage.md §4.1.1`; a successful build or source grep alone does not prove that events fire.
...
- A live site's Pixel initialization or consent semantics were changed without browser verification; `PageView` is missing or duplicated on a hard load; `ViewContent` is absent on chapter mount; or a route tracker sends an initial-mount/consent-change `PageView` in addition to the layout bootstrap.
