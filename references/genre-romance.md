...
## The Romance Click Standard

Every scene must contain **tension + question + desire, all three simultaneously**. Romance requires all three:

- A scene with only desire is erotica.
- A scene with only tension is thriller.
- A scene with only question is mystery.
- Romance requires all three — the reader must want the characters to close the distance (desire), feel something preventing it (tension), and not know if it will happen (question).

For non-romance genres that use a reduced set, see `genre-mystery.md` (mystery/thriller/gothic: tension + question only) or the relevant genre file.

**Regency exception:** desire is present but must be displaced — expressed through gesture, language choice, and proximity rather than stated directly. The social constraint is what makes the desire legible.
...
These apply only when the book sub-genre matches. Skip if the sub-genre is not listed.
...
### Applying to Parallel Writing (Romance)
...
**Default: steamy / closed-door at peak, non-graphic.** Build tension with sensation, breath, skin, pressure — fade at the peak intimate moment; stop before explicit sexual acts.
...
## Synopsis Formulas (Romance)

Use these for book descriptions and taglines. Apply the same hook standard as the cover: tension + question + desire, all three.
